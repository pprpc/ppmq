...
type PPMQGetSublistClient interface {
	PPCall(ctx context.Context, in *Req) (*Resp, *packets.CmdPacket, error)
}

type pPMQGetSublistClient struct {
	cc pprpc.RPCCliConn
}

func NewPPMQGetSublistClient(cc pprpc.RPCCliConn) PPMQGetSublistClient {
	return &pPMQGetSublistClient{cc}
}

func (c *pPMQGetSublistClient) PPCall(ctx context.Context, in *Req) (*Resp, *packets.CmdPacket, error) {
