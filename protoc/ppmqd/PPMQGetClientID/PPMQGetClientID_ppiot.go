...
const (
	CmdID   uint64 = 11
	CmdName string = "PPMQGetClientID"
	Module  string = "ppmqd"
)
...
type PPMQGetClientIDClient interface {
	PPCall(ctx context.Context, in *Req) (*Resp, *packets.CmdPacket, error)
}

type pPMQGetClientIDClient struct {
	cc pprpc.RPCCliConn
}

func NewPPMQGetClientIDClient(cc pprpc.RPCCliConn) PPMQGetClientIDClient {
	return &pPMQGetClientIDClient{cc}
}

func (c *pPMQGetClientIDClient) PPCall(ctx context.Context, in *Req) (*Resp, *packets.CmdPacket, error) {
