...
type PPMQUnSubClient interface {
	PPCall(ctx context.Context, in *Req) (*Resp, *packets.CmdPacket, error)
}

type pPMQUnSubClient struct {
	cc pprpc.RPCCliConn
}

func NewPPMQUnSubClient(cc pprpc.RPCCliConn) PPMQUnSubClient {
	return &pPMQUnSubClient{cc}
}

func (c *pPMQUnSubClient) PPCall(ctx context.Context, in *Req) (*Resp, *packets.CmdPacket, error) {
