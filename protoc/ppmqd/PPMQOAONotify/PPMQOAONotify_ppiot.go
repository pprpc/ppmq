...
type PPMQOAONotifyClient interface {
	PPCall(ctx context.Context, in *Req) (*Resp, *packets.CmdPacket, error)
}

type pPMQOAONotifyClient struct {
	cc pprpc.RPCCliConn
}

func NewPPMQOAONotifyClient(cc pprpc.RPCCliConn) PPMQOAONotifyClient {
	return &pPMQOAONotifyClient{cc}
}

func (c *pPMQOAONotifyClient) PPCall(ctx context.Context, in *Req) (*Resp, *packets.CmdPacket, error) {
