...
const (
	CmdID   uint64 = 23
	CmdName string = "PPMQEXChangeMsg"
	Module  string = "ppmqd"
)
...
type PPMQEXChangeMsgClient interface {
	PPCall(ctx context.Context, in *Req) (*Resp, *packets.CmdPacket, error)
}

type pPMQEXChangeMsgClient struct {
	cc pprpc.RPCCliConn
}

func NewPPMQEXChangeMsgClient(cc pprpc.RPCCliConn) PPMQEXChangeMsgClient {
	return &pPMQEXChangeMsgClient{cc}
}

func (c *pPMQEXChangeMsgClient) PPCall(ctx context.Context, in *Req) (*Resp, *packets.CmdPacket, error) {
