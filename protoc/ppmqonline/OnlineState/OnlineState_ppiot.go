...
const (
	CmdID   uint64 = 49
	CmdName string = "OnlineState"
	Module  string = "ppmqonline"
)
...
type OnlineStateClient interface {
	PPCall(ctx context.Context, in *Req) (*Resp, *packets.CmdPacket, error)
}

type onlineStateClient struct {
	cc pprpc.RPCCliConn
}

func NewOnlineStateClient(cc pprpc.RPCCliConn) OnlineStateClient {
	return &onlineStateClient{cc}
}

func (c *onlineStateClient) PPCall(ctx context.Context, in *Req) (*Resp, *packets.CmdPacket, error) {
